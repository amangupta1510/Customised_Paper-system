...
<link rel="stylesheet" type="text/css" href="{{ asset('table/css/mainnormal_paper_list_recycle_admin.css')}}">
<div class="limiter">
    <div class="title">
        <div class="searchBox">
            <h4><b>Deleted Papers</b>
                <div class="searchForm">
                    <form action="{{route('admin-recycle_custom_paper')}}" method="get">
    ...
    $(document).on('click', '.pagination a', function(e) {
        ...
        var img = "{{ route('admin-recycle_custom_paper_page',['s'=>':year','page'=>':page']) }}";
    ...
    $('#delete-student').click(function() {
        ...
        $.ajax({
            type: 'POST',
            url: '{{ route('
            admin - recycle_custom_paper_submit ') }}',
            data: {
    ...
    $('#p_delete-student').click(function() {
        ...
        $.ajax({
            type: 'POST',
            url: '{{ route('
            admin - p_delete_custom_paper_submit ') }}',
            data: {
                '_token': $('input[name=_token]').val(),
                'id': $('#p_delete-id').text(),
            },
